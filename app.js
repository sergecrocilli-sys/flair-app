...
function afficherInvitationRecue(invitation) {
  ...
  const regionLabel = managerLabel(invitation.region, 'non renseignée');
  ...
  if (text) {
    text.textContent = `Votre accès est préparé pour l’équipe ${teamLabel}, région ${regionLabel}. Créez votre compte avec l’email invité pour finaliser le rattachement.`;
  }
  ...
  if (region) {
    region.textContent = `Équipe ${teamLabel} · Région ${regionLabel}`;
  }
...
async function initUser() {
  ...
  chargerProfilMetier(currentProfil.profil_metier || 'agro_pesage');
  afficherApplication();
}
...
function afficherOnboardingMetier(profil = {}) {
  ...
  if (onboardingText) {
    if (invitationCourante?.team_id || profil.team_id) {
      ...
      const regionLabel = managerLabel(regionValue, 'non renseignée').replaceAll('_', ' ');

      onboardingText.textContent =
        `Bienvenue ${prenomLabel}, vous rejoignez l’équipe ${teamLabel}. ` +
        `Votre rôle est ${fonctionLabel} et votre région est préconfigurée : ${regionLabel}. ` +
        `Vérifiez les informations puis validez votre cockpit.`;
    } else {
      onboardingText.textContent =
        `Configurez votre environnement métier afin que FLAIR détecte les signaux les plus pertinents pour votre activité dans la région : ${managerLabel(regionValue, 'non renseignée').replaceAll('_', ' ')}`;
    }
  }
}
...
function afficherApplication() {
  ...
  if (cockpitRegionLabel) {
    ...
    cockpitRegionLabel.textContent = regionPrincipale
      ? `📍 Région principale : ${managerLabel(regionPrincipale, 'non renseignée').replaceAll('_', ' ')}`
...
function extraireRegionDepuisSignalTexte(s = {}) {
  ...
  const match = String(texte || '').match(/(?:r[eé]gion|region)\s*[:：]\s*([^\n|]+)/i);
  if (!match) return '';
...
function signalRegion(s) {
  return s.region || s.region_signal || s.zone || extraireRegionDepuisSignalTexte(s) || '';
}
...
function normaliserRegionImport(value) {
  return nettoyerValeurImport(value)
    .replace(/\s*[.;,]\s*$/, '')
    .trim();
}
...
function extraireSignalDepuisArticle(texteBrut) {
  ...
  const region = extraireRegionImportDepuisTexte(texte);
  ...
  const resultatEnrichi = enrichirScoringAvecSourceVeille({
    ...
    contenu: texte,
    resume: pourquoi,
    tags: [typeLibre, opportunite, actionRapide].filter(Boolean).join(' '),
    type_source: 'manuel'
  }, resultatInitial);
...
async function insererSignalAvecFallback(payload) {
  ...
  // toutes les colonnes préparatoires. Important : on teste séparément region
  // et region_signal pour conserver la région dès qu'une des deux colonnes existe.
  ...
  if (payload.region && payload.region_signal) {
  ...
  // - region absente mais region_signal existe
  // - region_signal absente mais region existe
  ...
  ajouterVariante(supprimerChamps(payload, ['region', 'region_signal', 'texte_original']));
  ajouterVariante(supprimerChamps(payload, ['region', 'region_signal', 'description']));
...
async function analyserArticleImporte() {
  ...
  const regionFinale = signalImporte.region || regionProfil;
  ...
  if (regionFinale) {
    ...
    // - region : nom cible prévu
    // - region_signal : ancien/alternatif déjà pris en charge par signalRegion()
