...
function afficherInvitationRecue(invitation) {
  ...
  if (text) {
    text.textContent = regionsSecondairesLabel
      ? `Votre accès est préparé pour l’équipe ${teamLabel}. Région principale : ${regionLabel}. Régions secondaires : ${regionsSecondairesLabel}. Créez votre compte avec l’email invité pour finaliser le rattachement.`
      : `Votre accès est préparé pour l’équipe ${teamLabel}, région principale ${regionLabel}. Créez votre compte avec l’email invité pour finaliser le rattachement.`;
  }
  ...
  if (region) {
    region.textContent = regionsSecondairesLabel
      ? `Équipe ${teamLabel} · Région principale ${regionLabel} · Secondaires ${regionsSecondairesLabel}`
      : `Équipe ${teamLabel} · Région principale ${regionLabel}`;
  }
  ...
  if (onboardingText) {
    if (invitationCourante?.team_id || profil.team_id) {
      ...
      onboardingText.textContent =
        ...
        `Votre rôle est ${fonctionLabel} et votre région principale est préconfigurée : ${regionLabel}. ` +
...
async function garantirContexteSignal() {
  ...
  if (!profil?.id || profil.id !== user.id || profil.team_id === undefined) {
    const { data, error } = await supabaseClient
      .from('commerciaux')
      .select('id, email, prenom, nom, role, team_id, region, regions_secondaires')
      .eq('id', user.id)
      .maybeSingle();
