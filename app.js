...
function famillesProjetsFlair() {
  ...
  return {
    extension: {
      ...
      keywords: ['extension', 'agrandissement', 'nouvelle usine', 'nouvelle ligne', 'augmentation capacite']
    },
    qualite: {
      label: 'Qualité / contamination / rappel',
      keywords: ['rappel produit', 'contamination', 'corps etranger', 'particules metalliques', 'particules de metal']
    },
    packaging: {
      label: 'Packaging / emballage',
      keywords: ['nouveau film', 'nouvel emballage', 'changement materiau', 'carton emballage', 'film barriere']
    },
    process: {
      label: 'Process / flux / fin de ligne',
      keywords: ['convoyage', 'accumulation', 'palettisation', 'flux de transfert', 'dispatching', 'sequencage']
    }
  };
}
...
function memeEntrepriseAntiDoublon(a, b) {
  const na = normaliserTexteAntiDoublon(a);
  const nb = normaliserTexteAntiDoublon(b);
...
async function rechercherProjetDejaDetecte(signalImporte, contexteSignal) {
  ...
  if (error) {
    console.warn('Anti-doublon indisponible :', error.message);
    return null;
  }

  const doublon = (data || []).find(signalExistant => {
  ...
  const dateReference = doublon.date_signal || doublon.created_at;
...
function messageProjetDejaDetecte(doublon) {
  ...
  const dateLabel = doublon.ageJours === null
    ? 'date non précisée'
    : `il y a ${doublon.ageJours} jour${doublon.ageJours > 1 ? 's' : ''}`;

  return `⚠ Projet déjà détecté : ${familleLabel}. Signal similaire déjà présent (${dateLabel}) : ${titre}. Ne pas supprimer : ce nouveau signal peut enrichir le suivi du projet.`;
}
