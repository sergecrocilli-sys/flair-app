...
function capScoreSelonCompatibilite(score, compatibilite, profilCommercial, profilPrincipal, profilsDetectes = []) {
  if (!profilsDetectes.length || !profilCommercial) return score;

  // Profil principal ou très compatible : pas de limitation.
  if (profilCommercial === profilPrincipal || compatibilite >= 0.85) return score;

  // Profil compatible mais secondaire : chaud possible, mais moins prioritaire.
  if (compatibilite >= 0.6) return Math.min(score, 90);

  // Profil indirect.
  if (compatibilite > 0) return Math.min(score, 79);
...
function construireLectureMetier(enrichissement = {}, profilCommercial = '') {
  ...
  const sousProfils = construireResumeSousProfils(enrichissement);
  const regles = construireResumeReglesMetier(enrichissement);

  if (!profils.length) return '';

  const profilsLabel = profils.map(labelProfilMetierRadar).join(', ');
  const principalLabel = profilPrincipal ? labelProfilMetierRadar(profilPrincipal) : 'Non défini';

  let niveauCompatibilite = '';
  if (profilCommercial) {
    const profilCommercialLabel = labelProfilMetierRadar(profilCommercial);

    if (profilCommercial === profilPrincipal || compatibilite >= 0.85) {
      niveauCompatibilite = `Compatibilité forte avec le profil ${profilCommercialLabel}.`;
    } else if (compatibilite >= 0.6) {
      niveauCompatibilite = `Compatible avec le profil ${profilCommercialLabel}, mais lecture principale ${principalLabel}.`;
    } else if (compatibilite > 0) {
      niveauCompatibilite = `Intérêt indirect pour le profil ${profilCommercialLabel}.`;
    } else {
      niveauCompatibilite = `Peu compatible avec le profil ${profilCommercialLabel}.`;
    }
  }

  return [
    `Lecture métier : ${principalLabel} principal.`,
    `Profils concernés : ${profilsLabel}.`,
    sousProfils ? `Sous-profils : ${sousProfils}.` : '',
    niveauCompatibilite,
    regles ? `Règles détectées : ${regles}.` : ''
  ].filter(Boolean).join(' ');
}

function ajouterPhraseMetier(texte, lectureMetier) {
  if (!lectureMetier) return texte || '';
  const base = String(texte || '').trim();
  if (!base) return lectureMetier;
  if (base.includes('Lecture métier :')) return base;
  return `${lectureMetier} ${base}`;
}

function construireAngleMetier(enrichissement = {}) {
  const profilPrincipal = enrichissement.profil_metier_principal || '';
  const sousProfils = construireResumeSousProfils(enrichissement, 4);

  if (!profilPrincipal && !sousProfils) return '';

  return [
    profilPrincipal ? `Angle métier : ${labelProfilMetierRadar(profilPrincipal)}.` : '',
    sousProfils ? `Technologies repérées : ${sousProfils}.` : ''
  ].filter(Boolean).join(' ');
}
...
function construirePrioriteMetier(enrichissement = {}, profilCommercial = '') {
  ...
  const commercialLabel = labelProfilMetierRadar(profilCommercial);
  const principalLabel = profilPrincipal ? labelProfilMetierRadar(profilPrincipal) : '';

  if (profilCommercial === profilPrincipal || compatibilite >= 0.85) {
    return `Priorité métier : forte compatibilité avec ${commercialLabel}.`;
  }

  if (compatibilite >= 0.6) {
    return `Priorité métier : signal compatible ${commercialLabel}, mais lecture principale ${principalLabel}.`;
  }

  if (compatibilite > 0) {
    return `Priorité métier : intérêt indirect pour ${commercialLabel}.`;
  }

  return `Priorité métier : faible compatibilité avec ${commercialLabel}.`;
}
...
function enrichirScoringAvecSourceVeille(signal, resultatInitial) {
  ...
  const profilPrincipal = enrichissement.profil_metier_principal || '';
  const compatibilite = compatibiliteMetierPourProfil(enrichissement, profilCommercial);
  const facteurBonus = facteurBonusSelonCompatibilite(compatibilite, profilsDetectes);
