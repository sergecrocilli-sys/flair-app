...
async function recupererTop3Actuel() {
  ...
  return {
    data: dedoublonnerSignauxPourAffichage([...(distributions.data || []), ...(anciensResult.data || [])]).slice(0, 3),
    error: null
  };
}
...
async function chargerAContacter() {
  ...
  const signaux = dedoublonnerSignauxPourAffichage([...(distributions.data || []), ...(data || [])]);
...
async function chercherNouvellesOpportunitesIA() {
  ...
  try {
    ...
    const lignesDistribution = signauxCompatibles.map(signal => ({
      ...
      raison_distribution: 'Distribution IA selon métier, région principale et régions secondaires.'
    }));
    ...
    const { error: traceError } = await supabaseClient
      ...
      .insert([{
        commercial_id: contexte.commercial_id,
        nb_signaux_distribues: signauxCompatibles.length
      }]);
    ...
    alert(`${signauxCompatibles.length} nouvelle${signauxCompatibles.length > 1 ? 's' : ''} opportunité${signauxCompatibles.length > 1 ? 's' : ''} IA ajoutée${signauxCompatibles.length > 1 ? 's' : ''} au radar.`);
  } finally {
    if (bouton) {
