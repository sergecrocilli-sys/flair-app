...
function signalRegion(s) {
  return s.region_nom || s.region || s.region_signal || s.zone || extraireRegionDepuisSignalTexte(s) || '';
}
...
async function insererSignalAvecFallback(payload) {
  ...
  // ni description/texte_original/tags, on conserve quand même la région dans un champ
  ...
  if (regionFallback && payload.entreprise_nom) {
    ...
    ajouterVariante(supprimerChamps(entrepriseAvecRegion, ['region_nom', 'region', 'region_signal', 'description', 'texte_original', 'tags']));
    ajouterVariante(supprimerChamps(entrepriseAvecRegion, ['region_nom', 'region', 'region_signal', 'description', 'texte_original', 'tags', 'date_signal']));
  }
...
async function analyserArticleImporte() {
  ...
  if (regionFinale) {
    ...
    // - tags : fallback si cette colonne existe déjà dans certains schémas
    payload.region_nom = regionFinale;
    payload.region = regionFinale;
    payload.region_signal = regionFinale;
    payload.tags = `Région : ${regionFinale}`;
  }
