...
async function chargerSignaux() {
  ...
  const signauxFiltres = (data || []).filter(signal => {
...
async function chercherNouvellesOpportunitesIA() {
  ...
  try {
    ...
    // Le bouton libère une courte sélection de signaux préparés en réserve IA
    // et compatibles avec le commercial connecté.
    let query = supabaseClient
      .from('signaux')
      .select('*')
      .eq('statut', 'reserve_ia')
      .or(`commercial_id.is.null,commercial_id.eq.${contexte.commercial_id}`)
    ...
    const ids = signaux.map(signal => signal.id);
    const nowIso = new Date().toISOString();
    ...
    if (updateError) {
      alert('Erreur activation opportunités IA : ' + updateError.message);
      return;
    }

    await analyserNouveauxSignaux({ silent: true, refresh: false, max: ids.length });
    await refreshCockpit();

    alert(`${ids.length} nouvelle${ids.length > 1 ? 's' : ''} opportunité${ids.length > 1 ? 's' : ''} IA ajoutée${ids.length > 1 ? 's' : ''} au radar.`);
  } finally {
    if (bouton) {
