...
function signalRegion(s) {
  return s.region || s.region_signal || s.zone || '';
}
...
function renderSignalCard(s, options = {}) {
  ...
  return `
    <div class="signal-card">
      ...
      ${region ? `<small><b>Région :</b> ${region}</small><br>` : ''}
...
function extraireSignalDepuisArticle(texteBrut) {
  ...
  const region = extraireChampStructure(texte, ['Région', 'Region', 'Zone']);
...
async function insererSignalAvecFallback(payload) {
  const variantes = [
    payload,
    (() => { const p = { ...payload }; delete p.region; return p; })(),
    (() => { const p = { ...payload }; delete p.region; delete p.date_signal; return p; })()
  ];
