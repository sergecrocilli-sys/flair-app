...
function calculerTimingCommercial(signal = {}, resultat = {}) {
  ...
  // Timing V2 — score dédié au timing.
  // score = qualité de la fenêtre commerciale (0 à 100)
  // impact_score = influence raisonnable sur le score global FLAIR
  ...
  const tropTard = [
    'inaugure', 'inauguree', 'inauguration', 'a inaugure',
    'mise en service realisee', 'mise en service réalisée',
    ...
    'fournisseur retenu', 'attribution du marche', 'attribution du marché',
    'marche attribue', 'marché attribué', 'contrat attribue', 'contrat attribué'
  ];

  const urgence03 = [
    'appel offre', 'appel d offres', 'appel d offre', 'appel offres',
    'consultation', 'consultations', 'consultation fournisseur', 'consultation fournisseurs', 'consultations fournisseurs', 'lancement prochain des consultations', 'dce', 'cahier des charges',
    'recherche fournisseur', 'recherche fournisseurs', 'demande de prix', 'demande de devis',
    'travaux demarres', 'travaux démarrés', 'chantier lance', 'chantier lancé',
    'installation en cours', 'mise en service imminente', 'mise en service prochaine',
  ...
  const ideal36 = [
    'investissement valide', 'investissement validé', 'budget valide', 'budget validé',
    'permis accepte', 'permis accepté', 'permis obtenu', 'permis de construire obtenu',
    'travaux prevus', 'travaux prévus', 'lancement des travaux', 'debut des travaux',
    'début des travaux', 'choix des equipements', 'choix des équipements',
    'selection fournisseurs', 'sélection fournisseurs', 'preparation consultation',
    'préparation consultation', 'prochains mois', 'dans les prochains mois', 'choix techniques', 'choix techniques devraient etre arretes', 'choix techniques devraient être arrêtés'
  ];
  ...
  if (contient(tropTard)) {
  ...
  } else if (contient(urgence03)) {
    phase = 'urgence_0_3_mois';
    score = 95;
    impact_score = 18;
    fenetre = '0-3 mois — agir vite';
    raison = 'Fenêtre courte : consultation, chantier, mise en service imminente ou urgence qualité détectée.';
  } else if (contient(ideal36)) {
    ...
    raison = 'Projet industriel annoncé : fenêtre favorable pour qualifier le besoin avant figement technique.';
  ...
  return { phase, score, impact_score, fenetre, raison };
}
...
function detecterSecteurSousSecteur(signal = {}) {
  ...
  const rules = [
    // Agroalimentaire
    { secteur: 'Agroalimentaire', sous: 'Viande / salaison', mots: ['viande', 'charcuterie', 'salaison', 'abattoir', 'volaille', 'boucherie', 'découpe viande'] },
    { secteur: 'Agroalimentaire', sous: 'Fromage / produits laitiers', mots: ['fromage', 'fromagerie', 'laiterie', 'produits laitiers', 'affinage'] },
    { secteur: 'Agroalimentaire', sous: 'Plats cuisinés / traiteur', mots: ['plats cuisines', 'plats cuisinés', 'traiteur', 'snacking', 'pret a manger', 'prêt à manger', 'barquettes alimentaires'] },
    { secteur: 'Agroalimentaire', sous: 'Fruits et légumes', mots: ['fruits', 'legumes', 'légumes', 'station de conditionnement', 'calibrage', 'maraicher'] },

    // Pharma / cosmétique
    { secteur: 'Pharmaceutique', sous: 'Conditionnement pharma', mots: ['pharma', 'pharmaceutique', 'médicament', 'medicament', 'laboratoire pharmaceutique', 'formes seches', 'formes sèches', 'injectable'] },
    { secteur: 'Cosmétique', sous: 'Conditionnement cosmétique', mots: ['cosmetique', 'cosmétique', 'parfum', 'parfumerie', 'beaute', 'beauté', 'maquillage', 'flacon', 'tube cosmetique'] },

    // Plasturgie / packaging
    { secteur: 'Plasturgie', sous: 'Extrusion / film plastique', mots: ['plasturgie', 'extrusion', 'film technique', 'film plastique', 'compound', 'granules', 'granulés', 'film barriere', 'film barrière'] },
    { secteur: 'Plasturgie', sous: 'Thermoformage / injection', mots: ['thermoformage', 'thermoformeuse', 'injection plastique', 'presse a injecter', 'presse à injecter', 'soufflage plastique'] },
    { secteur: 'Packaging', sous: 'Film / flowpack / operculage', mots: ['flowpack', 'flow pack', 'operculage', 'opercule', 'barquette', 'skin pack', 'emballage flexible'] },
    { secteur: 'Packaging', sous: 'Carton / étuis / conditionnement secondaire', mots: ['carton', 'cartonnage', 'etui', 'étui', 'encartonnage', 'conditionnement secondaire', 'caisse carton', 'mise en carton'] },
    { secteur: 'Packaging', sous: 'Étiquettes / sleeves / traçabilité', mots: ['etiquette', 'étiquette', 'sleeve', 'manchon', 'tracabilite', 'traçabilité', 'marquage', 'code barre', 'qr code'] },

    // Industrie transverse
    { secteur: 'Bois', sous: 'Scierie / panneaux / palettes', mots: ['scierie', 'bois', 'panneaux bois', 'palettes', 'sciage', 'rabotage', 'menuiserie industrielle'] },
    { secteur: 'Textile', sous: 'Textile technique / non-tissé', mots: ['textile', 'non tisse', 'non-tissé', 'fibres', 'recyclage textile', 'textile technique'] },
    { secteur: 'Chimie', sous: 'Process chimique / conditionnement', mots: ['chimie', 'chimique', 'poudres chimiques', 'conditionnement chimique'] },
    { secteur: 'Logistique', sous: 'Plateforme / flux internes', mots: ['plateforme logistique', 'logistique interne', 'entrepot', 'entrepôt', 'flux logistique'] }
  ];

  const match = rules.find(rule => rule.mots.some(mot => motCleFlairPresent(texte, mot)));
...
function interlocuteursPourProfil(profil = '', signal = {}) {
  ...
  if (type === 'appel_offre') {
    interlocuteurs.push('Responsable achats', 'Acheteur industriel', 'Responsable projet', 'Directeur industriel');
  }

  if (profil === 'detection') {
    interlocuteurs.push('Responsable qualité', 'Responsable production', 'Responsable maintenance', 'Directeur industriel', 'Directeur de site');
  } else if (profil === 'packaging') {
    interlocuteurs.push('Responsable conditionnement', 'Responsable packaging', 'Achats packaging', 'Responsable production', 'Responsable achats', 'Directeur industriel');
  } else if (profil === 'pesage') {
    interlocuteurs.push('Responsable qualité', 'Responsable production', 'Responsable méthodes', 'Responsable maintenance', 'Responsable amélioration continue');
  } else if (profil === 'vision') {
    interlocuteurs.push('Responsable qualité', 'Responsable production', 'Responsable automatisme', 'Responsable maintenance', 'Responsable méthodes');
  } else if (profil === 'process') {
    interlocuteurs.push('Responsable production', 'Responsable maintenance', 'Responsable travaux neufs', 'Responsable méthodes', 'Directeur industriel', 'Directeur de site');
  } else {
    interlocuteurs.push('Responsable qualité', 'Responsable production', 'Responsable maintenance', 'Directeur industriel');
  }

  if (secteur.includes('bois') || sous.includes('scierie')) {
    interlocuteurs.push('Directeur de scierie', 'Directeur de site');
  }

  if (secteur.includes('plasturgie') || sous.includes('extrusion') || sous.includes('injection')) {
    interlocuteurs.push('Responsable extrusion', 'Responsable injection', 'Responsable industrialisation');
  }

  if (secteur.includes('packaging') || sous.includes('conditionnement')) {
    interlocuteurs.push('Responsable conditionnement', 'Responsable packaging', 'Responsable achats');
  }

  if (signalSemblePME(signal)) {
    interlocuteurs.push('Dirigeant', 'Gérant', 'Président', 'Directeur général');
  }
...
function calculerScoreDistributionIA(signal = {}) {
  ...
  const copilote = preparerCopiloteCommercial(signal, {
    ...resultat,
  ...
  const enrichissementMetier = window.FLAIR_SOURCE_VEILLE?.analyserSignalAvecRegles
    ? window.FLAIR_SOURCE_VEILLE.analyserSignalAvecRegles(signal)
    : null;
...
function coefficientGeographiqueSignal(signal = {}) {
  ...
  // Règle FLAIR : France entière / national / multi-sites = zone commerciale neutre.
  // Le signal peut être proposé à tous les commerciaux, sans bonus ni malus géographique.
  if (estRegionNationaleFlair(regionSignalCle)) {
    return { coefficient: 1, niveau: 'national_signal' };
  }

  if (estRegionNationaleFlair(regionCommercialeCle)) {
    return { coefficient: 1, niveau: 'national_commercial' };
  }
...
function construirePhraseGeographique(ajustement = {}) {
  if (!ajustement || ajustement.niveau === 'non_renseigne') return '';
