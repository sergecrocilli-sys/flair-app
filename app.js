...
function renderBlocCopilotePremium(copiloteJson = {}) {
  const qui = normaliserListeCopilote(copiloteJson.qui_contacter);
  const lignesQui = qui.length
    ? `<ul style="margin:6px 0 0 16px;padding:0;">${qui.slice(0, 6).map(item => `<li>${item}</li>`).join('')}</ul>`
    : '<small>À qualifier.</small>';

  return `
    <div style="margin:10px 0;padding:12px 13px;border:1px solid rgba(59,130,246,0.42);background:linear-gradient(180deg,rgba(59,130,246,0.12),rgba(15,23,42,0.08));border-radius:12px;">
      <b>🧭 Copilote commercial</b><br>
      ${copiloteJson.pourquoi ? `<small><b>Pourquoi ?</b> ${copiloteJson.pourquoi}</small><br>` : ''}
      <small><b>Qui ?</b></small>${lignesQui}
      ${copiloteJson.vigilance ? `<small><b>Vigilance :</b> ${copiloteJson.vigilance}</small><br>` : ''}
      ${copiloteJson.timing ? `<small><b>Timing :</b> ${copiloteJson.timing}</small><br>` : ''}
