...
async function insererSignalAvecFallback(payload) {
  ...
  const ajouterVariante = (variante) => {
    const signature = JSON.stringify(Object.keys(variante).sort());
  ...
  // Si la table ne possède pas encore toutes les colonnes préparatoires,
  // on tente des variantes de plus en plus simples sans bloquer l'import.
  if (payload.texte_original) {
    const p = { ...payload };
    delete p.texte_original;
    ajouterVariante(p);
  }
  ...
  if (payload.region || payload.description || payload.texte_original) {
    const p = { ...payload };
    delete p.region;
    delete p.description;
    delete p.texte_original;
  ...
  if (payload.date_signal) {
    const p = { ...payload };
    delete p.region;
    delete p.description;
    delete p.texte_original;
    delete p.date_signal;
    ajouterVariante(p);
  }
...
async function analyserArticleImporte() {
  ...
  if (regionFinale) payload.region = regionFinale;
